...
class ActivityModule(private val appCompatActivity: AppCompatActivity) {
    @Provides
    @Singleton
    fun provideMatchCentreViewModel(factory: ViewModelFactory)
            = ViewModelProviders.of(appCompatActivity,factory)
        .get(MatchCentreViewModel::class.java)

    @Provides
    @Singleton
    fun provideRepository(matchService: MatchService):Repository
    = RepositoryImpl(matchService)
}